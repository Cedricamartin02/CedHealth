...
def goals():
    ...
    if request.method == 'POST':
        ...
        c.execute('INSERT INTO goals (user_id, weight_goal, calorie_goal) VALUES (?, ?, ?)',
...
def analyze_meal():
    ...
    if request.method == 'POST':
        ...
        if meal_name:
            try:
                response = requests.post(API_URL, headers=HEADERS, json={"query": meal_name})
                data = response.json()
                if 'foods' in data:
                    food = data['foods'][0]
                    nutrition_data = {
    ...
    return render_template('analyze_meal.html', nutrition_data=nutrition_data, error_message=error_message)
...
def meals():
    ...
    c = conn.cursor()
    if date_filter:
        c.execute('SELECT * FROM meals WHERE user_id = ? AND date = ?', (user_id, date_filter))
    else:
        c.execute('SELECT * FROM meals WHERE user_id = ?', (user_id,))
    meals_by_user = c.fetchall()
    conn.close()

    return render_template('meals.html', meals=meals_by_user, selected_date=date_filter)
...
def delete_meal(meal_id):
    ...
    conn = sqlite3.connect('cedhealth.db')
    c = conn.cursor()
    c.execute('DELETE FROM meals WHERE id = ? AND user_id = ?', (meal_id, session['user_id']))
    conn.commit()
    conn.close()
